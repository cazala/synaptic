...
export function buildGrowingNeuralCaHeapLayout(options: {
  ...
  const lengths: Readonly<Record<GrowingNeuralCaHeapSectionName, number>> = {
    ...
    // Each state channel owns one loss slot, avoiding atomics when objectives
    // supervise visible and hidden channels differently.
    loss: batchCells * channels,
...
export function measureGrowingNeuralCaState(
  ...
  for (let cell = 0; cell < target.length / 4; cell += 1) {
    ...
    if (
      ...
      const difference = alpha - targetAlpha;
      shapeError += Math.abs(difference) + difference * difference;
    }
    if (
      targetAlpha !== undefined &&
      Number.isFinite(targetAlpha) &&
...
function resolveOptions(options: GrowingNeuralCaTrainerOptions): ResolvedOptions {
  ...
  return {
    ...
    minRolloutSteps,
    stabilitySteps,
    shapeLossWeight: finiteRange(
      "shapeLossWeight",
      options.shapeLossWeight ?? 1,
      0,
      1_000,
    ),
...
fn reduceLoss() {
  var sum = 0.0;
  for (var index = 0u; index < BATCH_CELLS * CHANNELS; index += 1u) {
    sum += heap[LOSS + index];
  }
  heap[METRICS] = sum;
}
